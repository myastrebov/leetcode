...
public class RemoveDuplicatesFromSortedArray {
    public int removeDuplicates(int[] nums) {
        int current = nums[0];
        int numsCursor = 1;
        int resultArrayCursor = 1;
        while (numsCursor < nums.length) {
            int item = nums[numsCursor];
            if (nums[numsCursor] != current) {
                current = nums[numsCursor];
                nums[resultArrayCursor++] = current;
            }
            numsCursor++;
        }

        return resultArrayCursor;
    }
}
